...
(() => {
  ...
  var CompiledModel = class {
    constructor(model2, liteRtCompiledModel, options, onDelete) {
      ...
      this.options = options;
      this.onDelete = onDelete;
      const numSignatures = model2.liteRtModel.getNumSignatures();
  ...
  async function inferBitmap(model2, bitmap) {
    ...
    try {
      const outputs = await model2.run(inputTensor);
  ...
  function letterbox(img, imgW, imgH, targetW, targetH) {
    ...
    for (let i = 0; i < targetH * targetW; i++) {
      ...
      pixels[targetH * targetW + i] = rgba[i * 4 + 1] / 255;
  ...
  function scaleBox(x1, y1, x2, y2, confidence, classId, imgW, imgH, modelW, modelH, pad) {
